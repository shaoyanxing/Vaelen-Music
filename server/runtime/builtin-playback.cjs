// 服务端内置源播放/歌词/封面实现 —— 浏览器/Node 通用版本。
// 移植自 src/music-sdk/playback.js（前端），用 Node httpRequest 替换 lxRequest。
const sdk = require('./builtin-sdk.cjs')
const { httpRequest, toMD5, aes128EcbHex, eapi, zzcSign, mgSignature } = sdk

const DEFAULT_UA = 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/120.0.0.0 Safari/537.36'

// 通用 GET
const httpGet = (url, headers = {}) => httpRequest(url, { method: 'GET', headers: { 'User-Agent': DEFAULT_UA, ...headers } }).then(r => r.body)
// 通用 POST 表单
const httpPostForm = (url, form, headers = {}) => httpRequest(url, { method: 'POST', form, headers: { 'User-Agent': DEFAULT_UA, ...headers } }).then(r => r.body)
// 通用 POST JSON
const httpPostJson = (url, body, headers = {}) => httpRequest(url, { method: 'POST', body, headers: { 'User-Agent': DEFAULT_UA, 'Content-Type': 'application/json', ...headers } }).then(r => r.body)

// ============================================================================// WY
// ============================================================================
const WY_EAPI_URL = 'http://interface.music.163.com/eapi/batch'
const WY_LEVEL = {
  '128k': 'standard', '192k': 'higher', '320k': 'exhigh',
  flac: 'lossless', flac24bit: 'hires', hires: 'hires', jymaster: 'jymaster', sky: 'sky', jyeffect: 'jyeffect',
}
...
async function wyGetUrl(musicInfo, quality) {
  const id = musicInfo.songmid || musicInfo.id || (musicInfo.meta && musicInfo.meta.songId)
  ...
  for (const lv of levels) {
    try {
      const body = await httpPostForm(WY_EAPI_URL, eapi('/api/song/enhance/player/url/v1', {
        ids: [String(id)], level: lv, encodeType: 'flac', immerse: 0,
      }), { origin: 'https://music.163.com' })
      const data = body?.data?.[0] || body?.song?.[0]
      if (data && data.url && data.code === 200) return data.url
    } catch (e) { /* try next */ }
  }
...
async function wyGetLyric(musicInfo) {
  ...
  const body = await httpPostForm(WY_EAPI_URL, eapi('/api/song/lyric/v1', {
    id: String(id), tv: 0, lv: 0, kv: 0, rv: 0, yv: 0, showLyric: 1,
  }), { origin: 'https://music.163.com' })
  const lyric = body?.lrc?.lyric || body?.lyric
  if (!lyric) throw new Error('无歌词')
  return { lyric, tlyric: body?.tlyric?.lyric || '' }
}

function wyGetPic(musicInfo) {
  if (musicInfo.img) return Promise.resolve(musicInfo.img)
  const id = musicInfo.albumId || (musicInfo.meta && musicInfo.meta.albumId)
...
async function txGetUrl(musicInfo, quality) {
  ...
  const strMediaMid = musicInfo.strMediaMid || (musicInfo.meta && musicInfo.meta.strMediaMid)
  ...
  try {
    ...
    const resp = await httpPostJson(`https://u.y.qq.com/cgi-bin/musics.fcg?sign=${sign}`, reqBody, { 'User-Agent': 'QQMusic 14090508(android 12)' })
    const midInfo = resp?.req_0?.data?.midurlinfo?.[0]
    if (midInfo?.purl) return `https://isure.stream.qqmusic.qq.com/${midInfo.purl}&guid=${guid}&vkey=&uin=0&fromtag=120042`
    // 104003 = VIP/付费；不抛硬错误，让上层回退到第三方脚本尝试
  } catch (e) {
    throw e
  }
...
async function txGetLyric(musicInfo) {
  ...
  const body = await httpGet(`https://c.y.qq.com/lyric/fcgi-bin/fcg_query_lyric_new.fcg?format=json&inCharset=utf-8&outCharset=utf-8&notice=0&platform=yqq&needNewCode=0&songmid=${songmid}`, {
    Referer: 'https://y.qq.com/', 'User-Agent': 'QQMusic 14090508(android 12)',
  })
...
function txGetPic(musicInfo) {
  if (musicInfo.img) return Promise.resolve(musicInfo.img)
  const albumMid = musicInfo.albumMid || (musicInfo.meta && musicInfo.meta.albumMid)
...
// ============================================================================// KW
// ============================================================================
const KW_BR_MAP = { '128k': '128kmp3', '192k': '192kmp3', '320k': '320kmp3', flac: '2000kflac', flac24bit: '4000kflac' }
...
async function kwGetUrl(musicInfo, quality) {
  ...
  const body = await httpGet(`http://www.kuwo.cn/api/v1/www/music/playUrl?mid=${id}&type=convert_url&br=${br}`, { Referer: 'http://www.kuwo.cn/' })
...
async function kwGetLyric(musicInfo) {
  const id = musicInfo.songmid || musicInfo.id
  if (!id) throw new Error('缺少KW歌曲ID')
  const body = await httpGet(`http://m.kuwo.cn/newh5/singles/songinfoandlrc?musicId=${id}&httpsStatus=1`, { Referer: 'http://m.kuwo.cn/' })
  const lrcList = body?.data?.lrclist
  if (!Array.isArray(lrcList) || lrcList.length === 0) throw new Error('无歌词')
  const lyric = lrcList.map(l => `[${l.time || '00:00.00'}]${l.lineLyric || ''}`).join('\n')
  return { lyric, tlyric: '' }
}
...
async function kgGetUrl(musicInfo, quality) {
  const hash = musicInfo.hash || (musicInfo.meta && musicInfo.meta.hash)
  const albumId = musicInfo.albumId || (musicInfo.meta && musicInfo.meta.albumId) || ''
  if (!hash) throw new Error('缺少酷狗歌曲hash')
  const body = await httpGet(`https://wwwapi.kugou.com/yy/index.php?r=play/getdata&hash=${hash}&album_id=${albumId}&mid=&_=${Date.now()}`, { Referer: 'https://www.kugou.com/' })
...
async function kgGetLyric(musicInfo) {
  const hash = musicInfo.hash || (musicInfo.meta && musicInfo.meta.hash)
  if (!hash) throw new Error('缺少酷狗歌曲hash')
  const search = await httpGet(`https://wwwapi.kugou.com/search/songsearch_cdn?ver=1&hash=${hash}&keyword=&platform=WebFilter&area_code=1`, { Referer: 'https://www.kugou.com/' })
...
async function mgGetUrl(musicInfo, quality) {
  const copyrightId = musicInfo.copyrightId || (musicInfo.meta && musicInfo.meta.copyrightId)
  ...
  const playUrl = body?.data?.url
  if (!playUrl) throw new Error('咪咕播放地址获取失败')
  return playUrl
}

async function mgGetLyric(musicInfo) {
  const lrcUrl = musicInfo.lrcUrl || (musicInfo.meta && musicInfo.meta.lrcUrl)
  const copyrightId = musicInfo.copyrightId || (musicInfo.meta && musicInfo.meta.copyrightId)
  if (lrcUrl) {
...
const builtinGetMusicUrl = async (sourceId, musicInfo, quality) => {
...
const builtinGetLyric = async (sourceId, musicInfo) => {
  const impl = playbackImpls[sourceId]
  if (!impl?.getLyric) throw new Error(`内置源 ${sourceId} 不支持歌词`)
  return await impl.getLyric(musicInfo)
}

const builtinGetPic = async (sourceId, musicInfo) => {
  const impl = playbackImpls[sourceId]
  if (!impl?.getPic) return null
  try { return await impl.getPic(musicInfo) } catch { return null }
}

module.exports = { builtinGetMusicUrl, builtinGetLyric, builtinGetPic }
