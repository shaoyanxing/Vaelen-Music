// 前端歌单/排行榜 SDK —— 由 server/runtime/builtin-lists-kwkgmg.cjs（lx-music-desktop 官方源码移植版，已实网验证）忠实翻译为浏览器 ESM
// 源文件（master 分支）：
...
//   src/renderer/utils/musicSdk/mg/songList.js, mg/leaderboard.js, mg/musicInfo.js, mg/utils/index.js
//   src/common/utils/common.ts（dateFormat）, src/renderer/utils/index.ts（formatPlayCount）
// 替换依赖：httpRequest → lxRequest（http.js）；Node crypto → crypto-js；toMD5/mgSignature 复用 src/music-sdk/crypto.js；
// sizeFormate/formatPlayTime/decodeName/formatSingerName/formatSinger 复用 src/music-sdk/utils.js。
import CJS from './crypto-lib.js'
import { lxRequest } from './http.js'
import { toMD5, mgSignature } from './crypto.js'
...
// promise 解析为 { statusCode, headers, body }；前端无需真正取消请求。
const httpFetch = (url, options) => {
  const promise = lxRequest(url, options)
  return {
    promise,
    cancelHttp() {},
  }
...
// 服务端 Node crypto aes-128-ecb 的 crypto-js 等价实现
const wbdCrypto = {
  aesMode: 'aes-128-ecb',
  aesKey: CJS.enc.Latin1.parse(String.fromCharCode(112, 87, 39, 61, 199, 250, 41, 191, 57, 68, 45, 114, 221, 94, 140, 228)),
  aesIv: '',
  appId: 'y67sprxhhpws',
  decodeData(base64Result) {
    // crypto-js 4.x 对传入 WordArray ciphertext 的解密会返回空，需直接传 base64 字符串
    const data = CJS.AES.decrypt(
      decodeURIComponent(base64Result),
      this.aesKey,
      { mode: CJS.mode.ECB, padding: CJS.pad.Pkcs7 },
  ...
  buildParam(jsonData) {
    const data = CJS.AES.encrypt(
      CJS.enc.Utf8.parse(JSON.stringify(jsonData)),
      this.aesKey,
      { mode: CJS.mode.ECB, padding: CJS.pad.Pkcs7 },
    ).ciphertext
    const time = Date.now()

    const encodeData = CJS.enc.Base64.stringify(data)
